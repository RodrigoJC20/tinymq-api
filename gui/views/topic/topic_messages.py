...
class TopicMessagesView(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(header_frame, text=f"Messages for Topic: {self.topic_id}", font=("Helvetica", 16, "bold"))
        ...
        self.messages_tree = ttk.Treeview(
            list_frame,
            columns=("id", "publisher", "published_at", "size", "preview"),
            show="headings",
            selectmode="browse"
        )
        ...
        self.messages_tree.heading("preview", text="Payload Preview")
        
        self.messages_tree.column("id", width=50)
        self.messages_tree.column("publisher", width=200)
        self.messages_tree.column("published_at", width=150)
        self.messages_tree.column("size", width=100)
        self.messages_tree.column("preview", width=300)
    ...
    def _fetch_messages(self, skip):
        ...
        try:
            messages = self.api_client.get_messages_by_topic(self.topic_id, skip=skip, limit=self.page_size)
            self.after(0, lambda: self._update_messages_list(messages))
        except Exception as e:
            self.after(0, lambda: messagebox.showerror("Error", f"Failed to load messages: {e}"))
    ...
    def _update_messages_list(self, messages):
        ...
        for msg in messages:
            ...
            self.messages_tree.insert(
                "", "end", 
                values=(msg.id, msg.publisher_client_id, published_at, msg.payload_size, msg.payload_preview)
            )
