...
class SubscriptionTopicView(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(header_frame, text="Subscription Topic Details", font=("Helvetica", 16, "bold"))
        ...
        ttk.Label(details_frame, text="Number of Subscriptions:").grid(row=4, column=0, sticky="e", padx=5, pady=2)
        self.subscription_count_label = ttk.Label(details_frame, text="")
        self.subscription_count_label.grid(row=4, column=1, sticky="w", padx=5, pady=2)
    ...
    def _fetch_topic_details(self):
        topic = self.api_client.get_topic_by_subscription(self.subscription_id)
        if topic:
            subscriptions = self.api_client.get_subscriptions_by_topic(topic.id)
            ...
            self.subscription_count_label.config(text=len(subscriptions))
